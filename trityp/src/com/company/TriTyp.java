...
public class TriTyp {
    ...
    public void retornaTrianguloUsu() {
        int[] entries = readTriSides();
        T = triang(entries[0], entries[1], entries[2]);
        System.out.println("Result is: " + triTypes[T]);
    }

    public int[] readTriSides(){
    ...
    public int triang(int Side1, int Side2, int Side3) {
        ...
        if (Side2 == Side3)
            triOut = triOut + 3;
        if (triOut == 0) {  // Confirm it's a legal triangle before declaring
            // it to be scalene
...
// Read (or choose) an integer
    public int getN() {
        ...
        } catch (IOException e) {  // JDK requires the IOException to be caught.
